...
export type SerbianWordClassification = {
  ...
  lemma?: string;
  /**
   * The word is a real Serbian word, but in the Ijekavian (Montenegrin)
   * dialect, not the Ekavian teaching norm. Holds the corresponding Ekavian
   * form, e.g. "gdje" -> "gde". The surface itself is never booked as a new
   * vocabulary headword; see checkVocabularyCoverage.nonStandardDialectForms.
   */
  ijekavianEkavian?: string;
};
 ...
 * The base prompt (chatPrompts `cs_validator_classifier`) and the shared
 * Serbian language rules (`cs_language_rules`) are mandatory: the classifier
 * must judge Ekavian vs. Ijekavian by the same norm as every other stage.
 */
export async function classifyAndTranslateWords(
  ...
  const basePrompt = await resolvePromptFromDb(ctx, CS_PROMPT_KEYS.validatorClassifier);
  ...
  // Same retry and provider fallback as the Translator: a 503 "high demand"
  // from the provider is transient. If every attempt fails, the error is
  // thrown to the caller; there is no result without a classification.
  const primaryProvider = pickPrimaryProvider(undefined);
  const { raw } = await callTextRobust(
    ctx,
    { step: "validator:classify", stage: "specialist", system, user: userPrompt, maxTokens: 2000 },
    { primaryProvider, fallbackProvider: pickFallbackProvider(primaryProvider) },
  );
  ...
  try {
    ...
    throw new Error(
      `Word classifier returned invalid JSON (${String((err as Error)?.message || err)}). Response start: ${cleanedRaw.slice(0, 200)}`
    );
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error(`Word classifier returned no word map. Response start: ${cleanedRaw.slice(0, 200)}`);
  }
  ...
  for (const [word, info] of Object.entries(parsed)) {
    ...
    } else if (lang === "ijekavian" && typeof data?.ekavian === "string" && data.ekavian.trim()) {
      const ekavianForm = normalizeSerbianKey(data.ekavian);
      if (ekavianForm && ekavianForm !== key) {
        result.set(key, { isSerbian: false, ijekavianEkavian: ekavianForm });
      } else {
...
export async function checkVocabularyCoverage(
   ...
   * Ekavian teaching norm requires the Ekavian form. Never booked as a new
   * vocabulary headword; the Fix stage replaces the surface in the text with
   * the Ekavian form. The variant may still appear in Notes via the
   * Montenegrin-pronunciation skill, on the Ekavian entry.
   */
  nonStandardDialectForms: Array<{ surface: string; ekavianForm: string }>;
}> {
  ...
  // No verdict without a classification: an unclassified word is not a
  // missing Serbian word. A failed call is thrown to the caller.
  ...
  for (const candidate of candidatesToProcess) {
    ...
    // teaching norm is Ekavian: never book the surface as a new headword,
    // report it so the Validator can turn it into a blocking finding.
    if (classification?.ijekavianEkavian) {
      nonStandardDialectForms.push({ surface: candidate.surface, ekavianForm: classification.ijekavianEkavian });
      continue;
    }
...
export function buildAuditPayload(pkg: any, knownFromPreviousUnits?: string[]): any {
  ...
  return {
    ...
    contentEn: {
      ...
      // overviewMd includes the Cultural Note (appended at parse time, see
      // scripts/markdownParser/extractors.ts): the cap must fit both, or the
      // Lector never sees the Cultural Note and the Montenegrin-pronunciation
      // skill's variant notes on long overview text get cut off.
      overviewMd: truncateForAudit(contentEn?.overviewMd, 4000),
    ...
    // teaches it (2026-09-17). Capped at the same limit as the system
    // prompt's KNOWN_VOCAB_KEY_CAP (formatKnownVocabularyKeys) so the payload
    // never shows a shorter list than what the model was already told.
    knownFromPreviousUnits: Array.isArray(knownFromPreviousUnits)
      ? knownFromPreviousUnits.slice(0, KNOWN_VOCAB_KEY_CAP)
      : [],
    ...
    // a micro-unit stays well below it (<= 35 by curriculum rule). noteEn is
    // NOT truncated: the Montenegrin-pronunciation skill's "In Montenegro:
    // <form>." line lives at the end of noteEn and a 200-char cut silently
    // dropped it for entries with a longer Chunk/Gender/AlsoMeaning note.
    vocabularySample: Array.isArray(pkg?.vocabulary?.en)
      ? pkg.vocabulary.en.slice(0, 80).map((v: any) => ({
          serbian: String(v?.serbian || ""),
          en: String(v?.en || ""),
          noteEn: typeof v?.noteEn === "string" ? v.noteEn : undefined,
          gender: v?.gender,
        }))
      : [],
    exercises,
