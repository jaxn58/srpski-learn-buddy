...
export const SPECIALIST_SYSTEM_PROMPT = [
  ...
  `8. Dialect note (Montenegro): if the unit uses gde/gdje, mention the variant in Notes.`,
  `9. EXERCISES (CRITICAL): Do NOT repeat the **Instructions:** text inside each row. The instruction appears once per exercise, questions should be concise.`,
  `10. EXERCISES (QUALITY GATE): Within each exercise table, do NOT repeat the same question/sentence text. Avoid copy-paste stems; every row must be meaningfully different.`,
  ``,
  `═══════════════════════════════════════════════════════════════════════════`,
...
export const getAuditorSystemPrompt = (
  ...
  ``,
  `=== YOUR TASK ===`,
  `Return ONLY JSON: {"ok":boolean,"blockers":[...],"warnings":[...]}`,
  `Where blockers/warnings are objects {code,message,path?}.`,
  `IMPORTANT: Do NOT put raw line breaks inside JSON strings. Use \\n escapes.`,
  ``,
  `Task: Return ONLY JSON with keys:`,
  `{"ok":boolean,"blockers":[...],"warnings":[...]} `,
  `Where blockers/warnings are arrays of objects {code,message,path?}.`,
  ...
  `- Every BLOCKER must reference concrete evidence: include questionId(s) or a path like 'exercises.en[category=...]'.`,
  `- BLOCKERS are ONLY for objective rule violations:`,
  `  - missing required exercise categories, invalid blank format, missing/invalid options, correctAnswer missing/mismatching options, duplicate Serbian keys, fruit-as-drink, obvious Serbian correctness errors.`,
  `- Style/pedagogy suggestions (e.g., "too easy", "simple vocabulary translation") MUST be WARNINGS, not blockers.`,
  `Focus on:`,
  `- Serbian correctness (obvious errors), beginner suitability, consistency with Unit 1/2 style`,
  `- Forbidden patterns: duplicate Serbian keys, fruit-as-drink, missing options, wrong blank format`,
  `- Hallucination risk: avoid invented “facts”; keep neutral cultural notes`,
  auditSkillBlock ? `\n${auditSkillBlock}\n` : ``,
].join("\n");
