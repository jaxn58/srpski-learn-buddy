# External AI Prompt (Markdown Unit Authoring) — v2
...
**Changelog from v1:**
- Added explicit output-only constraint (no meta claims like “moved files/pushed”).
- Added clarifications for Unit 1 coverage, Vocabulary columns, required Description, and Progression block placement.
...
Vocabulary rules:
- One row = ONE Serbian entry with ONE meaning.
