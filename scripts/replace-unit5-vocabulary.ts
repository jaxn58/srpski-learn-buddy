/**
 * Replace Unit 5 vocabulary - Delete all existing and import new from Markdown file
 * 
 * Usage: npx tsx scripts/replace-unit5-vocabulary.ts
 */
...
const MARKDOWN_FILE = path.join(
  ...
  "New Content",
  "learn-with.me-main",
  "Unit-5-Finding-a-Place-to-Stay-v6.md"
);
...
/**
 * Parse markdown table and extract vocabulary words from Unit 5
 */
    ...
    if (inTable && line.startsWith("|") && headerFound) {
      ...
      if (cells.length >= 2) {
        ...
        if (serbian && english && serbian !== "Serbian" && english !== "English") {
          ...
          // Handle multiple entries separated by "/" (e.g., "Konobar / Konobarica")
          if (serbian.includes("/")) {
            ...
            // Check if English also has "/" (e.g., "Waiter / Waitress")
            ...
            parts.forEach((part, idx) => {
              ...
              if (englishParts.length > 0) {
                // Use corresponding English part
                englishPart = englishParts[idx] || englishParts[0];
              } else if (hasMfNote) {
              ...
              if (cleanPart && englishPart) {
                ...
                if (hasMfNote) {
                  gender = idx === 0 ? "m" : "f";
                } else if (notes?.toLowerCase().includes("male") && idx === 0) {
                  gender = "m";
                } else if (notes?.toLowerCase().includes("female") && idx === 1) {
                  gender = "f";
...
async function replaceUnit5Vocabulary() {
  console.log("🚀 Replacing Unit 5 Vocabulary...");
  ...
  // Step 1: Delete all existing Unit 5 vocabulary
  console.log("\n   Step 1: Deleting all existing Unit 5 vocabulary...");
  try {
  ...
  console.log(`\n📊 Total Unit 5 Vocabulary: ${allVocab.length} words`);
}

replaceUnit5Vocabulary().catch((error) => {
