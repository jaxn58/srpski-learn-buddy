...
// Generate week structure based on learning duration
export function generateWeekStructure(durationInWeeks: number) {
  const totalUnits = 27;
  const unitsPerWeek = totalUnits / durationInWeeks;
  
  const weeks = [];
  let currentUnit = 1;
  
  for (let week = 1; week <= durationInWeeks; week++) {
    const unitsThisWeek = [];
    const targetUnit = Math.ceil(week * unitsPerWeek);
    
    while (currentUnit <= targetUnit && currentUnit <= totalUnits) {
      unitsThisWeek.push(currentUnit);
      currentUnit++;
    }
    
    if (unitsThisWeek.length > 0) {
      weeks.push({
        weekNumber: week,
        units: unitsThisWeek
      });
...
// Calculate current week based on completed units and learning duration
export function calculateCurrentWeek(completedUnits: number[], learningDuration: number): number {
  const totalUnits = 27;
  const completedCount = completedUnits.length;
  const progressRatio = completedCount / totalUnits;
  const currentWeek = Math.max(1, Math.ceil(progressRatio * learningDuration));
